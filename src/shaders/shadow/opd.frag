...
float ShadowCalc(){
    ...
    float shadow = (currentDepth < depth + 1e-4) ? 1.0:0.1;
    return shadow;
}
void main()
{
    ...
    vec3 light_dir = vec3(view * vec4(light_pos, 1.0)) - FragPos;
    // vec3 light_dir = light_pos - FragPos;
    float distance = length(light_dir);
    distance = distance * distance;
    light_dir = normalize(light_dir);
    vec3 reflect_dir = reflect(-light_dir, normal);
    vec3 diffuse = max(dot(normal, light_dir), 0.0) * light_color * 2.0 / distance;

    float spec = pow(max(dot(view_dir,reflect_dir),0.0), 20.0) * 4.0;
